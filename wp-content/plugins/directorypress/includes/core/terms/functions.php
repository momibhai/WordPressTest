...
function directorypress_tax_dropdowns_menu_init($params) {
	...
	if ($terms) {
		...
		foreach ($terms AS $term) {
			...
			echo '<option id="' . esc_attr($term->slug) . esc_attr($option_id_inc) . '" value="' . esc_attr($term->term_id) . '" data-fonticolor="'. esc_attr($icon_color) .'" data-name="' . esc_attr($term->name)  . '" data-sublabel="" ' . wp_kses_post($selected) . ' ' . wp_kses_post($icon) . ' data-icontype="'. esc_attr($icon_type) .'" ' . wp_kses_post($term_count) . '>' . esc_html($term->name) . '</option>';
			if ($depth > 1) {
...
function _directorypress_tax_dropdowns_menu($tax, $parent = 0, $depth = 2, $current_level = 1, $term_id = null, $count = false, $exact_terms = array(), $hide_empty = false) {
	...
	if ($terms && ($depth == 0 || !is_numeric($depth) || $depth > $current_level)) {
		...
		if ($terms) {
			...
			$option_id_inc = uniqid();
				echo '<option id="' . esc_attr($term->slug) . esc_attr($option_id_inc) . '" value="' . esc_attr($term->term_id) . '" data-fonticolor="'. esc_attr($icon_color) .'" data-name="' . esc_attr($term->name)  . '" data-sublabel="' . esc_attr($sublabel) . '" ' . wp_kses_post($selected) . ' ' . wp_kses_post($icon) . ' data-icontype="' . esc_attr($icon_type) . '" ' . wp_kses_post($term_count) . '>' . esc_html($term->name) . '</option>';
