			...
			echo '<div class="directorypress-location-item col-md-' . esc_attr($instance->col) . ' col-sm-' . esc_attr($instance->col_tab) . ' col-xs-' . esc_attr($instance->col_mobile) . '">';	
				echo '<div class="directorypress-location-item-holder">';
					echo '<div class="directorypress-parent-location">';
						echo '<a href="' . get_term_link($term) . '" title="' . esc_attr($term->name) . esc_attr($term_count) . '"><span class="location-icon"><i class="fas fa-map-marker-alt"></i></span><span class="loaction-name">' . esc_html($term->name) .'</span><span class="location-item-numbers">'. esc_html($term_count) .'</span></a>';
					echo '</div>';
					if($instance->depth > 1){
