<?php
/**
 * HFB Advanced Headers Bar Post Meta Box
 *
 * @package   HFB Pro
 */
...
class HFB_Conditions_Setting {
	...
	private static $device_selection;
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_hfb_get_posts_by_query', array( $this, 'hfb_get_posts_by_query' ) );
	}
	...
	public function initialize_options() {
		...
		self::$device_selection     = self::get_device_selections();
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'hfb_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'header-footer-builder' ),
			'special-search' => __( 'Search Page', 'header-footer-builder' ),
			'special-blog'   => __( 'Blog / Posts Page', 'header-footer-builder' ),
			'special-front'  => __( 'Front Page', 'header-footer-builder' ),
			'special-date'   => __( 'Date Archive', 'header-footer-builder' ),
			'special-author' => __( 'Author Archive', 'header-footer-builder' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'header-footer-builder' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'header-footer-builder' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'header-footer-builder' ),
					'basic-singulars' => __( 'All Singulars', 'header-footer-builder' ),
					'basic-archives'  => __( 'All Archives', 'header-footer-builder' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'header-footer-builder' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'header-footer-builder' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'header-footer-builder' ),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'hfb_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'header-footer-builder' ),
				'value' => array(
					'all'        => __( 'All', 'header-footer-builder' ),
					'logged-in'  => __( 'Logged In', 'header-footer-builder' ),
					'logged-out' => __( 'Logged Out', 'header-footer-builder' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'header-footer-builder' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'hfb_user_roles_list', $selection_options );
	}
	 ...
	 * @return array
	 */
	public static function get_device_selections() {
		$selection_options = array(
			'all' => __( 'All', 'header-footer-builder' ),
			'desktop'  => __( 'Desktop', 'header-footer-builder' ),
			'mobile' => __( 'Tablet/Mobile', 'header-footer-builder' ),
		);
		return apply_filters( 'hfb_devices_list', $selection_options );
	}
	 ...
	 * @since  1.0.0
	 */
	function hfb_get_posts_by_query() {

		check_ajax_referer( 'hfb-get-posts-by-query', 'nonce' );
		...
		unset( $post_types['hfb-post'] ); //Exclude EHF templates.
	 ...
	 * This is only run from hfb_get_posts_by_query()
	 *
	...
	public function admin_styles() {
		wp_enqueue_script( 'hfb-select2', AST_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), AST_TARGET_RULE_VER, true );
		...
		wp_enqueue_script( 'hfb-target-rule' );
		...
		wp_enqueue_script( 'hfb-user-role' );

		wp_register_style( 'hfb-select2', AST_TARGET_RULE_URI . 'select2.css', '', AST_TARGET_RULE_VER );
		wp_enqueue_style( 'hfb-select2' );
		wp_register_style( 'hfb-target-rule', AST_TARGET_RULE_URI . 'target-rule.css', '', AST_TARGET_RULE_VER );
		wp_enqueue_style( 'hfb-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'header-footer-builder' ),
			'please_delete' => __( 'Please delete', 'header-footer-builder' ),
			'more_char'     => __( 'or more characters', 'header-footer-builder' ),
			'character'     => __( 'character', 'header-footer-builder' ),
			'loading'       => __( 'Loading more results…', 'header-footer-builder' ),
			'only_select'   => __( 'You can only select', 'header-footer-builder' ),
			'item'          => __( 'item', 'header-footer-builder' ),
			'char_s'        => __( 's', 'header-footer-builder' ),
			'no_result'     => __( 'No results found', 'header-footer-builder' ),
			'searching'     => __( 'Searching…', 'header-footer-builder' ),
			'not_loader'    => __( 'The results could not be loaded.', 'header-footer-builder' ),
			'search'        => __( 'Search pages / post / categories', 'header-footer-builder' ),
			'ajax_nonce'    => wp_create_nonce( 'hfb-get-posts-by-query' ),
		);
		wp_localize_script( 'hfb-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'header-footer-builder' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-hfb-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="hfb-target-rule-condition ast-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '<option value="">' . __( 'Select', 'header-footer-builder' ) . '</option>';
		...
		$output .= '</div> <!-- hfb-target-rule-condition -->';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'header-footer-builder' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'header-footer-builder' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'header-footer-builder' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="hfb-target-rule-condition ast-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'header-footer-builder' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'hfb_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-hfb-user-role-condition">';
			$output     .= '<div class="hfb-user-role-condition ast-user-role-{{data.id}}" data-rule="{{data.id}}" >';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- hfb-user-role-condition -->';
		$output             .= '</script>';
		...
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="hfb-user-role-condition ast-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
	...
	/**
	 * Parse device condition.
	 *
	 * @since  1.0.0
	 * @param  int   $post_id Post ID.
	 * @param  Array $rules   Current devices.
	 *
	 * @return boolean  True = condition passes. False = condition does not pass.
	 */
	public function parse_device_condition( $post_id, $rules ) {
		$show_popup = true;
	...
	/**
	 * Function Name: target_devices_settings_field.
	 * Function Description: Function to handle new input type.
	 *
	 * @param string $name string parameter.
	 * @param string $settings string parameter.
	 * @param string $value string parameter.
	 */
	public static function target_devices_settings_field( $name, $settings, $value ) {
		$input_name     = $name;
		$type           = isset( $settings['type'] ) ? $settings['type'] : 'target_rule';
		$class          = isset( $settings['class'] ) ? $settings['class'] : '';
		$rule_type      = isset( $settings['rule_type'] ) ? $settings['rule_type'] : 'target_rule';
		...
		if ( ! isset( self::$device_selection ) || empty( self::$device_selection ) ) {
			self::$device_selection = self::get_device_selections();
		}
		$selection_options = self::$device_selection;
		...
		$output         .= '<div class="ast-device-wrapper ast-device-display-on-wrap" data-type="display">';
			$output     .= '<div class="ast-device-selector-wrapper ast-device-display-on">';
				$output .= '<div class="device-builder-wrap">';
					foreach ( $saved_values as $index => $data ) {
						$output     .= '<div class="hfb-device-condition ast-device-' . $index . '" data-rule="' . $index . '" >';
							//$output .= '<span class="device-condition-delete dashicons dashicons-dismiss"></span>';
							/* Condition Selection */
							$output     .= '<div class="device-condition-wrap" >';
								$output .= '<select name="' . esc_attr( $input_name ) . '[' . $index . ']" class="device-condition form-control ast-input">';
									$output .= '<option value="">' . __( 'Select', 'header-footer-builder' ) . '</option>';

										//foreach ( $selection_options as $item => $item_data ) {
											//$output .= '<optgroup label="' . $group_data['label'] . '">';
												foreach ( $selection_options as $opt_key => $opt_value ) {
													$output .= '<option value="' . $opt_key . '" ' . selected( $data, $opt_key, false ) . '>' . $opt_value . '</option>';
												}
											//$output .= '</optgroup>';
										//}
								$output .= '</select>';
							$output     .= '</div>';
						$output .= '</div> <!-- hfb-device-condition -->';
					}
				$output .= '</div>';
			$output .= '</div>';
		$output .= '</div>';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'hfb_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
		...
		if ( false === $meta_header ) {
			...
			$this->remove_devices_posts( $post_type, $option );
		}
	 ...
	 * @param  array $option meta option name.
	 */
	public function remove_devices_posts( $post_type, $option ) {
		$devices           = isset( $option['devices'] ) ? $option['devices'] : '';
		$current_post_id = isset( $option['current_post_id'] ) ? $option['current_post_id'] : false;

		foreach ( self::$current_page_data[ $post_type ] as $c_post_id => $c_data ) {
			$selected_device = get_post_meta( $c_post_id, $devices, true );
			$is_device    = $this->parse_device_condition( $current_post_id, $selected_device );
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'header-footer-builder' ), $rule_set_titles );
	...
	public static function get_post_selection( $post_type ) {
		...
		if ( ! empty( $all_headers ) ) {
			$headers = array(
				'' => __( 'Select', 'header-footer-builder' ),
			);
 ...
 */
HFB_Conditions_Setting::get_instance();
