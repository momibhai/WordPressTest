   ...
   function main_shortcode_output( $atts, $content = null )
    {
	  ...
	  if ( ! difp_current_user_can('access_message') ){
	  	...
	  	return "<div class='alert alert-danger'>".__("You do not have permission to access message system", 'directorypress-frontend-messages')."</div>";
	  }
	  
	  $atts = shortcode_atts( array(
			'difpaction'		=> 'messagebox',
			'difp-filter'		=> 'show-all',
		), $atts, 'directorypress-frontend-messages' );
      ...
      { 
        $out = "<div class='alert alert-danger'>".sprintf(__("You must <a href='%s'>login</a> to view your message.", 'directorypress-frontend-messages'), wp_login_url( get_permalink() ) )."</div>";
      }
    ...
    function Header()
    {
	  ...
	  $max_text = $max_total ? number_format_i18n($max_total) : __('unlimited', 'directorypress-frontend-messages' );
...
function view_message()
    {
	  ...
	  if ( ! $id || ! difp_current_user_can( 'view_message', $id ) ) {
	  	return "<div class='alert alert-danger'>".__("You do not have permission to view this message!", 'directorypress-frontend-messages')."</div>";
	  }
