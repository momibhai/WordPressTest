...
function upload_file( $upload_data, $message_id, $inserted_message ) {
	...
	if ( $attachment_ids ) {
		  echo "<hr /><strong>" . __("Attachments", 'directorypress-frontend-messages') . ":</strong><br />";
		  foreach ( $attachment_ids as $attachment_id ){
		  
			$name = esc_html( basename(wp_get_attachment_url( $attachment_id )) );
		
			echo "<a href='".difp_query_url('download', array( 'difp_id' => $attachment_id, 'token' => wp_create_nonce('download_' . $attachment_id ) ))."' title='". sprintf(__( 'Download %s', 'directorypress-frontend-messages'), $name ) . "'>{$name}</a><br />";
				} 
			}
		}
	...
	function download_file(){
		...
		wp_die(__('Invalid token', 'directorypress-frontend-messages'));
		
		if ( !difp_current_user_can( 'access_message' ) )
		wp_die(__('No attachments found', 'directorypress-frontend-messages'));
		...
		if( ! in_array( $post_type, array( 'difp_message', 'difp_announcement' ) ) ) {
			wp_die(__('You have no permission to download this attachment.', 'directorypress-frontend-messages'));
		...
		if( !file_exists($attachment_path) ){
			...
			wp_die(__('Attachment already deleted', 'directorypress-frontend-messages'));
		}
