...
class Difp_Shortcodes
  {
	...
	function message_to( $atts, $content = null ) {
		$atts = shortcode_atts( array(
				...
				'text'		=> __('Contact','directorypress-frontend-messages' ),
				'class'		=> 'difp-button'
	...
	function new_message_form( $atts, $content = null ){
		$atts = shortcode_atts( array(
				...
				'enable_ajax'		=> 'true',
				'heading'		=> __('Contact','directorypress-frontend-messages' )
	...
	function new_bidding_form( $atts, $content = null ){
		$atts = shortcode_atts( array(
			...
			if( '{current-post-id}' == $atts['listing_id'] ){
				//$page = get_post_ID_by_title(, OBJECT, 'directorypress-listing');
